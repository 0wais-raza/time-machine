...
export function HolographicNavBar() {
  ...
  const { activeTab, setActiveTab, tasks, prayers, prayerTimes, credits } = useApp();
      ...
      <div className="relative flex h-14 items-center justify-between gap-3 px-4">
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          <div className="relative flex size-7 shrink-0 items-center justify-center">
            <div className="absolute inset-0 animate-[holo-spin_7s_linear_infinite] rounded-full border-2 border-[oklch(0.85_0.17_200/0.35)]" style={{ borderTopColor: "var(--holo-cyan)" }} />
            <div className="absolute inset-[4px] animate-[holo-spin-rev_5s_linear_infinite] rounded-full border border-[oklch(0.66_0.27_295/0.4)]" style={{ borderBottomColor: "var(--holo-violet)" }} />
        ...
        <div className="flex items-center gap-2">
          <div className="hidden items-center gap-3 rounded-full border border-[oklch(0.85_0.17_200/0.12)] bg-[oklch(1_1_1/0.03)] px-3 py-1.5 xl:flex">
            <span className="font-mono-tech text-xs font-bold tabular-nums text-foreground" suppressHydrationWarning>
              {now
                ? `${String(now.getHours() % 12 || 12).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`
                : "--:--"}
            </span>
            <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[var(--holo-cyan)]">{np?.name ?? ""} {npStr}</span>
          </div>
          <div className="flex items-center gap-1.5 rounded-full border border-[oklch(0.82_0.16_80/0.22)] bg-[oklch(0.82_0.16_80/0.07)] px-2.5 py-1.5" title="Cyber Credits">
            <Coins className="size-3.5 text-[var(--holo-amber)]" />
            <span className="font-mono-tech text-xs font-bold tabular-nums text-[var(--holo-amber)]" suppressHydrationWarning>
              {now ? credits : 0}
            </span>
          </div>
          <div className="hidden items-center gap-1.5 rounded-full border border-[oklch(0.8_0.16_155/0.22)] bg-[oklch(0.8_0.16_155/0.07)] px-2.5 py-1.5 md:flex" title="Streak">
            <Flame className="size-3.5 text-[var(--holo-green)]" />
            <span className="font-mono-tech text-xs font-bold tabular-nums text-[var(--holo-green)]" suppressHydrationWarning>
              {now ? streak : 0}
            </span>
          </div>
          <div className="hidden items-center gap-1.5 rounded-full border border-[oklch(0.66_0.27_295/0.22)] bg-[oklch(0.66_0.27_295/0.07)] px-2.5 py-1.5 md:flex" title="Namaz">
            <Moon className="size-3.5 text-[var(--holo-violet)]" />
            <span className="font-mono-tech text-xs font-bold tabular-nums text-[var(--holo-violet)]">{prayerDone}/5</span>
          </div>
          <NotificationBell />
        </div>
      </div>
